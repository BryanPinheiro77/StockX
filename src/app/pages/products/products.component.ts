...
export class ProductsComponent implements OnInit {
  ...
  nomeEmpresa: string = '';  
  limiteEstoqueCritico: number = 2;  // padrão, será sobrescrito pelo Firebase
  ...
  async ngOnInit(): Promise<void> {
    onAuthStateChanged(this.auth, async (user) => {
      if (user) {
        const docRef = doc(this.firestore, `usuarios/${user.uid}/configuracoes/${user.uid}`);
        const snapshot = await getDoc(docRef);
        if (snapshot.exists()) {
          const data = snapshot.data();
          this.nomeEmpresa = data['nomeEmpresa'] || '';
          this.limiteEstoqueCritico = data['estoqueCritico'] ?? 2;
        }
  ...
  async atualizarDados() {
    ...
    console.log('Produtos carregados:', this.produtos); // DEBUG
    this.produtosFiltrados = [...this.produtos];
    const resumo = await this.productService.getResumo();
    this.resumo = resumo;

    const produtosComEstoqueBaixo = this.produtos.filter(p => (p.quantity || 0) <= this.limiteEstoqueCritico);
